...
export default function ProductGallery({ media = [] }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="relative group w-full h-[320px] min-h-[320px] lg:h-[500px] lg:min-h-[500px] bg-slate-900 rounded-2xl border border-border shadow-sm overflow-hidden">
        {/* Media Counter */}
        ...
        {items[activeIndex] && items[activeIndex].type !== "youtube" && (
          <div className="absolute bottom-4 right-4 bg-black/60 backdrop-blur-xs text-white px-2.5 py-1 rounded-md text-[11px] font-semibold flex items-center gap-1.5 shadow-sm select-none pointer-events-none transition-opacity duration-250 opacity-0 group-hover:opacity-100 z-10">
          ...
          <div className="flex h-full w-full items-center">
            {items.map((item, index) => {
              ...
              return (
                <div
                  key={index}
                  className="flex-[0_0_100%] min-w-0 h-full relative select-none flex items-center justify-center bg-slate-900"
                >
                  <div className="w-full h-full flex items-center justify-center relative">
                    <img
                      ...
                      className={`w-full h-full object-cover select-none transition-transform duration-250 ${
                    ...
                    {isYoutube && (
                      <>
                        <div 
                          className="absolute inset-0 bg-black/20 hover:bg-black/35 transition-colors duration-250 flex items-center justify-center cursor-pointer"
        ...
        {showNavButtons && (
          <>
            <button
              ...
              className="absolute left-6 top-1/2 -translate-y-1/2 w-12 h-12 rounded-full bg-slate-900/70 hover:bg-slate-900/80 backdrop-blur-md border border-white/10 shadow-lg flex items-center justify-center transition-all duration-250 active:scale-95 opacity-100 lg:opacity-0 lg:group-hover:opacity-100 lg:-translate-x-2 lg:group-hover:translate-x-0 z-10 cursor-pointer"
          ...
          {items.map((item, index) => {
            ...
            return (
              <button
                ...
                className={`relative w-20 sm:w-24 aspect-[4/3] rounded-lg border overflow-hidden shrink-0 transition-all duration-250 cursor-pointer ${
                  isActive
            ...
            if (slide.type === "youtube") {
              return (
                ...
                >
                  ...
                  />
                  <div className="absolute inset-0 bg-black/20 hover:bg-black/35 transition-colors duration-250 flex items-center justify-center">
